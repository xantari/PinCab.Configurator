...
namespace PinCab.Utils.Utils.DmdExt
{
    /// <summary>
    /// Small revision to the out of the box file ini processing so it doesn't add all the extra spaces to the comment lines
    /// </summary>
    public class CondensedFileIniFormatter : IIniDataFormatter
    {
        IniParserConfiguration _configuration;

        #region Initialization
        public CondensedFileIniFormatter() : this(new IniParserConfiguration()) { }

        public CondensedFileIniFormatter(IniParserConfiguration configuration)
        {
        ...
        private void WriteSection(SectionData section, StringBuilder sb)
        {
            ...
            //12/6/2020 - MRO: Remove these extra comment new lines
